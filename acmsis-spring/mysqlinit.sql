-- 创建数据库
CREATE
DATABASE IF NOT EXISTS labelclocksystem;
USE
labelclocksystem;
...
SELECT Id
FROM user
WHERE "1234567891" IN (StuNum, Name, StuCardNum);

-- 外键级联删除更新
INSERT INTO Record(UserId, StartTime, EndTime, Time)
VALUES (1, now(), now(), (UNIX_TIMESTAMP() - UNIX_TIMESTAMP()) / 3600);


INSERT INTO user(Name, StuNum, StuCardNum, Grade, Major)
VALUES ("张三", "202121091111", "1111111111", "2021", "计算机"),
...
ALTER TABLE UserStatus
    ADD CONSTRAINT UserStatusUserId FOREIGN KEY (UserId) REFERENCES User (Id) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE Record
    ADD CONSTRAINT RecordUserId FOREIGN KEY (UserId) REFERENCES User (Id) ON DELETE CASCADE ON UPDATE CASCADE;
